    ...
    <div class="container my-2">
        ...
        <div class="my-1 p-1 d-flex justify-content-between">
            ...
            @if ($blog->user->id === Auth::id())
                <button class="btn btn-sm btn-danger mx-1"><i class="fas fa-trash-alt"></i> Blog</button>
            @endif
        </div>
                    ...
                    @switch($content->type)
                        @case('text')
                            <button class="btn btn-sm btn-link text-primary" type="button" data-bs-toggle="modal"
                                data-bs-target="#textEditModal_{{ $content->id }}"><i class="far fa-edit align-middle"></i></button>
                            @include('blog.includes.edit-content', ['type' => 'text', 'id' => $content->id, 'content' => $content->content])
                        @break
                        @case('image')
                            <button class="btn btn-sm btn-link text-primary" type="button" data-bs-toggle="modal"
                                data-bs-target="#imageEditModal_{{ $content->id }}"><i class="far fa-edit align-middle"></i></button>
                            @include('blog.includes.edit-content', ['type' => 'image', 'id' => $content->id, 'content' => $content->content])
                        @break
                        @case('code')
                            <button class="btn btn-sm btn-link text-primary" type="button" data-bs-toggle="modal"
                                data-bs-target="#codeEditModal_{{ $content->id }}"><i class="far fa-edit align-middle"></i></button>
                            @include('blog.includes.edit-content', ['type' => 'code', 'id' => $content->id, 'content' => $content->content])
                        @break
                        @default
